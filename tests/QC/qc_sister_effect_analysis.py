...
Usage:
    ...
    python ops_process/tests/QC/qc_sister_effect_analysis.py -e 89

    # All experiments, pooled across experiments (default, memory-efficient):
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad

    # All experiments, per-experiment (old behavior):
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad --per-experiment

    # Custom min cells per bin:
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad --min-cells 50

    # Force recompute even if a cached CSV exists:
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad --force

    # Submit as a SLURM job (400GB, 32 CPUs, 30 min):
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad --slurm

    # Submit and don't wait for completion:
    python ops_process/tests/QC/qc_sister_effect_analysis.py --all --remove-bad --slurm --no-wait
"""
...
from ops_utils.data.bad_experiments import DEFAULT_EXCLUDE_CATEGORIES, is_excluded
...
def run_analysis(args):
    ...
    if evo_path is None:
        candidates = [
            ...
            Path(__file__).resolve().parents[3] / "ops_process" / "cyclops_process" / "configs" / "evo_genes.csv",
        ]
