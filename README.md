...
[`czbiohub-sf/ops_monorepo`](https://github.com/czbiohub-sf/ops_monorepo) uv workspace and is
...
All commands are then run from the **monorepo root** (`ops_monorepo/`) with `uv run`, as in
every example below — never from inside a subpackage directory, never with a manually
activated conda/venv, and never with bare `python` or `pytest`. See the
[monorepo README](https://github.com/czbiohub-sf/ops_monorepo#getting-started) for cluster
setup (`module load uv`, `UV_CACHE_DIR`).
...
uv sync --extra tracking  # cell tracking (ultrack, gurobipy, higra)
...
This repository is owned by the [Leonetti group](https://biohub.org/leonetti/) at the
[Chan Zuckerberg Biohub San Francisco](https://github.com/czbiohub-sf).
