...
from cyclops_utils.data.disk_cache import df_cache
...
# ``cyclops_model/src/cyclops_model/models/attention/atlas/``; add that to
...
_ATLAS_DIR = (
    ...
    / "cyclops_model" / "src" / "cyclops_model" / "models" / "attention" / "atlas"
)
...
def _load_attention_table(csv_path: str | Path, gene: str) -> pd.DataFrame:
    ...
    /path/to/ops_data/cache/cyclops_utils/ root so all users on
...
def view_by_gene_attention(
    ...
    def _ntc_cache_path(exp: str, well_2lvl: str) -> Path:
        ...
        from cyclops_utils.data.disk_cache import _cache_path
