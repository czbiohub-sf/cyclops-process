...
from ops_utils.hpc.slurm_utils import check_jobs_complete
...
def submit_iss_registration_jobs(
    ...
    if slurm_params is None:
        # Get the ops_process directory for PYTHONPATH (3 levels up from this file)
        ops_process_dir = str(Path(__file__).parents[3])  # Go up to ops_process directory
        slurm_params = {
            "timeout_min": 10,
            "mem": "32GB",
            "cpus_per_task": 16,
            ...
            "slurm_setup": [f"export PYTHONPATH={ops_process_dir}:$PYTHONPATH"],  # Ensure cyclops_process is importable
        }
...
def monitor_and_finalize_wells(
    ...
    from ops_utils.hpc.slurm_utils import monitor_jobs_with_retry
...
def register_iss_cycles(
    ...
    if slurm_params is None:
        ...
        slurm_params = {
            ...
            "slurm_partition": "cpu",
            "slurm_srun_args": ["--cpu-bind=none"],  # Disable CPU binding to avoid non-contiguous CPU errors
            "slurm_setup": [f"export PYTHONPATH={ops_process_dir}:$PYTHONPATH"] + repro_setup,
        }
    elif reproducible:
