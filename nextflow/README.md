...
- In the `./nextflow/`* directory a nextflow run can be started with the following command:
    ...
    - This command will occupy the current terminal so we are currently looking at wrapping it in a `slurm` command so that it can asynchronously manage scheduling.
        ...
        ```
        sbatch --job-name=nf-iss --output=logs/nf-iss-%j.log --partition=cpu --cpus-per-task=2 --mem=8G --time=12:00:00 --wrap="source /path/to/ops_monorepo/.venv/bin/activate && nextflow run iss.nf -c iss.config -params-file nextflow_ops_args.yaml"
        ```
...
To point at a different source:
  ...
  1. OpsDataset (ops_utils/src/ops_utils/data/experiment.py, line 33) — change iss_tif_dir (and lc_dragonfly_dir if using live cell data) to point at the alternate
